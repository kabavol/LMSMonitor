...
inline socket_t create_client_socket(const char *host, int port,
    ...
    return create_socket(
        host, port, [&](socket_t sock, struct addrinfo &ai) -> bool {
            ...
            auto ret = ::connect(sock, ai.ai_addr,
...
class MultipartFormDataParser {
  public:
    ...
    bool parse(const char *buf, size_t n, T content_callback,
               U header_callback) {
...
inline bool write_multipart_ranges_data(Stream &strm, const Request &req,
    ...
    return process_multipart_ranges_data(
        ...
        [&](size_t offset, size_t length) {
            return write_content(strm, res.content_provider, offset, length) >=
                   0;
        });
}
...
inline void Response::set_content_provider(
    ...
    content_provider = [provider](size_t offset, size_t length,
                                  DataSink &sink) {
...
inline bool Server::write_response(Stream &strm, bool last_connection,
    ...
    if (res.body.empty()) {
        ...
        if (req.ranges.empty()) {
            ...
            res.body = detail::make_multipart_ranges_data(req, res, boundary,
                                                          content_type);
        }
...
inline int Server::bind_internal(const char *host, int port, int socket_flags) {
    ...
    if (port == 0) {
        ...
        if (address.ss_family == AF_INET) {
            return ntohs(
                reinterpret_cast<struct sockaddr_in *>(&address)->sin_port);
            ...
            ContentReader reader(
                [&](ContentReceiver receiver) {
                    return read_content_with_content_receiver(
                        strm, last_connection, req, res, receiver, nullptr,
                        nullptr);
                },
                [&](MultipartContentHeader header, ContentReceiver receiver) {
                    return read_content_with_content_receiver(
                        strm, last_connection, req, res, nullptr, header,
                        receiver);
                });
...
inline bool Server::process_and_close_socket(socket_t sock) {
    return detail::process_and_close_socket(
        false, sock, keep_alive_max_count_, read_timeout_sec_,
        read_timeout_usec_,
    ...
    while (i < requests.size()) {
        ...
        if (!process_and_close_socket(
                ...
                [&](Stream &strm, bool last_connection,
                    ...
                    auto &req = requests[i++];
                    auto res = Response();
                    auto ret = handle_request(strm, req, res, last_connection,
                                              connection_close);
                    if (ret) {
...
inline bool Client::handle_request(Stream &strm, const Request &req,
    ...
    if (!is_ssl() && !proxy_host_.empty()) {
        ...
        ret =
            process_request(strm, req2, res, last_connection, connection_close);
    } else {
        ret =
            process_request(strm, req, res, last_connection, connection_close);
    }
    ...
    if (res.status == 401 || res.status == 407) {
        ...
        if (!username.empty() && !password.empty()) {
            ...
            if (parse_www_authenticate(res, auth, is_proxy)) {
                Request new_req = req;
                auto key =
                    is_proxy ? "Proxy-Authorization" : "WWW-Authorization";
                new_req.headers.erase(key);
                new_req.headers.insert(make_digest_authentication_header(
                    req, auth, 1, random_string(10), username, password,
                    is_proxy));
...
inline std::shared_ptr<Response> Client::Post(const char *path,
                                              ...
                                              ContentProvider content_provider,
                                              const char *content_type) {
    return Post(path, Headers(), content_length, content_provider,
                content_type);
}
...
inline std::shared_ptr<Response> Client::Patch(const char *path,
                                               ...
                                               ContentProvider content_provider,
                                               const char *content_type) {
    return Patch(path, Headers(), content_length, content_provider,
                 content_type);
}
...
inline bool SSLServer::process_and_close_socket(socket_t sock) {
    return detail::process_and_close_socket_ssl(
        false, sock, keep_alive_max_count_, read_timeout_sec_,
        read_timeout_usec_, ctx_, ctx_mutex_, SSL_accept,
        [](SSL * /*ssl*/) { return true; },
