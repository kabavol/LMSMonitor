 ...
 */
static void bmp_create_standard_color_table(bmpfile_t *bmp) {
    int i, j, k, ell;
 ...
 */
static void bmp_create_grayscale_color_table(bmpfile_t *bmp) {
    int i;
    uint8_t step_size;
...
static void bmp_malloc_colors(bmpfile_t *bmp) {
    ...
    if (bmp->dib.depth == 1 || bmp->dib.depth == 4 || bmp->dib.depth == 8) {
        bmp->colors =
            (rgb_pixel_t *)malloc(sizeof(rgb_pixel_t) * bmp->dib.ncolors);
...
static void bmp_malloc_pixels(bmpfile_t *bmp) {
    ...
    bmp->pixels =
        (rgb_pixel_t **)malloc(sizeof(rgb_pixel_t *) * bmp->dib.width);
    for (i = 0; i < bmp->dib.width; ++i) {
        bmp->pixels[i] =
            (rgb_pixel_t *)malloc(sizeof(rgb_pixel_t) * bmp->dib.height);
...
uint32_t bmp_get_width(bmpfile_t *bmp) { return bmp->dib.width; }

uint32_t bmp_get_height(bmpfile_t *bmp) { return bmp->dib.height; }

uint32_t bmp_get_depth(bmpfile_t *bmp) { return bmp->dib.depth; }

bmp_header_t bmp_get_header(bmpfile_t *bmp) { return bmp->header; }

bmp_dib_v3_header_t bmp_get_dib(bmpfile_t *bmp) { return bmp->dib; }
...
static bool _is_big_endian(void) {
...
static void bmp_header_swap_endianess(bmp_header_t *header) {
...
static void bmp_dib_v3_header_swap_endianess(bmp_dib_v3_header_t *dib) {
...
static void bmp_get_row_data_for_1(bmpfile_t *bmp, unsigned char *buf,
    ...
    while (i < bmp->dib.width) {
        for (j = 0, index = 0; j < 8 && i < bmp->dib.width; ++i, ++j)
            index +=
                pos_weights[j] * find_closest_color(bmp, bmp->pixels[i][row]);

        buf[k++] = index & 0xff;
    }
}

static void bmp_get_row_data_for_4(bmpfile_t *bmp, unsigned char *buf,
                                   size_t buf_len, uint32_t row) {
...
static void bmp_get_row_data_for_8(bmpfile_t *bmp, unsigned char *buf,
                                   size_t buf_len, uint32_t row) {
    int i;
...
static void bmp_get_row_data_for_24(bmpfile_t *bmp, unsigned char *buf,
                                    size_t buf_len, uint32_t row) {
    int i;
...
static void bmp_get_row_data_for_32(bmpfile_t *bmp, unsigned char *buf,
                                    size_t buf_len, uint32_t row) {
    int i;
